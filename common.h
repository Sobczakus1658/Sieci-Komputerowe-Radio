...
int16_t readNumberAddress(char *address){
    std::string number = "";
    int counter = 0;
    while(*address != '.' && *address != ' ' && *address != '\0'){
    ...
    return std::strtoul(number.c_str(), nullptr, 10);
}
...
size_t read_message_udp(int socket_fd, struct sockaddr_in *client_address, char *buffer, size_t max_length) {
